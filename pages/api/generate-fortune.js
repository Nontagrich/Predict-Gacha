...
export default async function handler(req, res) {
  ...
  try {
    ...
    const bangkokDate = new Date(today.toLocaleString('en-US', { timeZone: 'Asia/Bangkok' }));
    const todayStr = bangkokDate.toISOString().split('T')[0]; // YYYY-MM-DD
    const todayFormatted = bangkokDate.toLocaleDateString('th-TH', { 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric' 
    });
    ...
    const prompt = [
      ...
      ``,
      `IMPORTANT RULES:`,
      `1. Each "character" field MUST contain ONLY ONE character name from this list: ${currentBanners.join(', ')}`,
      `2. Create 4-5 recommended dates within the next 2 weeks`,
      `3. ALL dates must be AFTER ${todayStr} (today's date)`,
      `4. Dates must be in YYYY-MM-DD format`,
      `5. Times must be in 24-hour format (HH:MM-HH:MM) and be reasonable (between 00:00-23:59)`,
      `6. Distribute the characters across different lucky days`,
      ``,
    ...
    fortune.luckyDays = fortune.luckyDays.filter(day => {
      const dayDate = new Date(day.date);
      return dayDate >= todayDate;
    });

    if (fortune.luckyDays.length === 0) {
      throw new Error('No valid future dates generated by AI');
    }
